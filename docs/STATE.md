...
- **`enabled: false` does not drop chunks already indexed.** The two
  `Assessment … Attempt review` PDFs were indexed before being disabled: 27
  chunks, still searchable, invisible to `review`. Either `forget` them or
  re-enable them — undecided. The fix in code is for ingest to drop what the
  manifest disables.
